...
public class Family implements Sortable, Cloneable {
	...
	// method can added members to members and parents array , and do that cant
	// recive more than two perants
	public boolean addMember(Person member, String role) {
		try {

			// this custom Exception (SameIDException) i created for check neither have the
			// same id
			...
			// this custom Exception (ParentsLimitException) i created for check if family
			// dosen't have more 2 parent
			...
			if (role.equals("mom") || role.equals("dad")) {
				...
				if (parents.size() >= 2) {
					...
					// this custom Exception (NoAddingParentsException) i created until user can't
					// add a son/daughter before mom and dad
	...
	// this method has arrays for sorted a families by number of Martyr (sort is
	// desc)
	@Override
